...
int EpollCtl::run() {
    ...
    arg->epfd = m_epfd;
    arg->map = &m_callback_map;

    thread_t tid;
    pthread_create(&tid, NULL, EpollCtl::epoll_maintain, arg);
}

void* EpollCtl::epoll_maintain(void *targ) {
    thread_arg* arg = (thread_arg*)targ;
    int ret;
    struct epoll_event events[512];
    printf("epfd = %d\n", arg->epfd);
    while (true) {
        ret = epoll_wait(arg->epfd, events, 512, 5000);
        if (ret < 0) {
